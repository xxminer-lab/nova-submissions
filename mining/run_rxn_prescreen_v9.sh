...
# Usage: run_rxn_prescreen_v9.sh <RXN_ID> <LIMIT> <OFFSET>
#
...
socket.setdefaulttimeout(90)  # a stalled read/connect raises -> fallback to next mirror
...
def dl(url, dest):
    ...
    return sz > 1000000
# GATEWAY FIRST (HF CDN/xethub redirect is degraded from Modal -> stalls); HF as fallback.
for name, urls in [
    ...
    ok = False
    # up to 3 attempts per file, cycling mirrors
    for attempt in range(3):
        for url in urls:
            try:
                ...
                print(f"failed {url} (attempt {attempt+1}): {type(e).__name__}: {e}", flush=True)
                try:
...
python3 /nova/score_candidates_incr.py --candidates /nova/candidates.tsv --out "/nova/scores_rxn${RXN_ID}.tsv" --target P40261 --limit "${LIMIT}" --offset "${OFFSET}" 2>&1 \
